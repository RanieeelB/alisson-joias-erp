...
add constraint payments_method_check check (
  method in ('wire', 'ach', 'credit_card', 'cash', 'check', 'pix')
);
...
begin
  if not exists (
    ...
    add constraint payments_status_check check (
      status in ('settled', 'pending_deposit', 'credit')
    );
  end if;
end $$;
...
create index if not exists declarations_customer_id_idx
  on public.declarations(customer_id);

alter table public.declarations enable row level security;

drop trigger if exists declarations_set_updated_at on public.declarations;
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'declarations'
      and policyname = 'declarations_internal_access'
  ) then
    create policy declarations_internal_access
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'declarations'
      and policyname = 'declarations_select_own_customer'
  ) then
    create policy declarations_select_own_customer
    on public.declarations
    for select
    to authenticated
    using (
      exists (
        ...
        where customers.id = declarations.customer_id
